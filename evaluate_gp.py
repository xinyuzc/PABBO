...
def evalate_on_a_gp_dataset(
    ...
    """Evaluate PABBO on one GP dataset with specified random seed.

    Args:
        Xopt, (num_global_optima, d_x): the global optima locations.
        yopt, (num_global_optima, 1): the global optima.
        sampler_kwargs, Dict:
            - kernel_function, str: in ["rbf", "matern12", "materin32", "matern52"]
            - mean, torch.Tensor(1)
            - jitter, float
        function_kwargs, Dict
        test_x_range, (d_x, 2)
        ...
        eval_num_query_points, int: the size of query set for PABBO.
    ...
    """
    set_all_seeds(seed)
    # visualize the trajectory on function
    if plot_freq > 0:
    ...
    d_x = Xopt.shape[-1]
    # Create test function. We first create GP sampler with defined kernel function and mean
    ...
    # finally create a sampler for the function
    ...
    # create the query set for PABBO
    X, y, _, _ = sampler.sample(
        batch_size=1,
        num_total_points=eval_num_query_points,
    ...
    ).bool()  # (1, num_query_pairs, 1), mask to register queried pairs

    # load defined initial pairs for the dataset
    (
        context_pairs,
        context_pairs_y,
        context_c,
    ...
    if logger is not None:
        logger.info(f"(Scaled) initial pairs: \n {context_pairs}")
        ...
        for t in range(1, eval_max_T):
            ...
            if plot_freq > 0 and (t % plot_freq == 0):
                ...
                if d_x == 1:
                    ...
                    plot_predictions_on_1d_function(
                        X=scale_from_domain_1_to_domain_2(
                            x=X[0].clone(), bound1=test_x_range, bound2=train_x_range
                        ),
...
def evaluate(config: DictConfig):
    ...
    logfilename = os.path.join(
        root, f'eval_{config.data.name}_{time.strftime("%Y%m%d_%H%M%S")}.log'
    )
