...
def train(config: DictConfig, model: TransformerModel):
    ...
    logfilename = os.path.join(root, f'train_{time.strftime("%Y%m%d_%H%M%S")}.log')
    ...
    # NOTE set seed before initializing the sampler to ensure the same split of traing samples for acq and pred tasks.
    set_all_seeds(seed)
    sampler = HPOBHandler(
        root_dir=osp.join(hydra.utils.get_original_cwd(), DATASETS_PATH, "hpob-data"),
    ...
    for epoch in range(start_step, config.train.n_steps + 1):
        ...
        if epoch > config.train.n_burnin:
            # sample points for query dataset
        ...
        if epoch % config.train.save_freq == 0 or epoch == config.train.n_steps:
            ...
            ckpt = {
                ...
                "expdir": expdir,
                "step": epoch + 1,
                "seed": config.train.train_seed,  # NOTE for recovering training data split for acq and pred tasks
            }
